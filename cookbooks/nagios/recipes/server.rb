...
if node['nagios']['server']['search_method'] == 'server'
  # use the users_helper.rb library to build arrays of users and contacts
  nagios_users = NagiosUsers.new(node)
else
  nagios_users={}
  nagios_users['users']=[]
  data_bag(node['nagios']['users_databag']).each do |i|
    u=data_bag_item(node['nagios']['users_databag'], i)
    if [u['groups']].flatten.include? node['nagios']['users_databag_group'] and not [u['action']].flatten.include? 'remove'
      nagios_users['users'] << u
    end
  end
end
