...
##       ebs_vol_config_string_list   = "${docker.template_file.all_ebs_volume_detail_id_lists.*.rendered}"
...
data "template_file" "all_ebs_volume_detail_id_lists" {
  ...
  template      = "${
    join(
      ";",
      list(
        "${element(module.ebs-vol-docker.volume_details_ids_list, count.index)}"
      )
    )
  }"
}
...
module "ebs-vol-docker" {
  ...
  count         = "${var.instance_count}"
  instance_ids  = "${module.instance_cluster.instance_ids}"
  ...
  vol_name      = "docker"      ##  IGNORED
  vol_label     = "docker"      ##  MUST BE UNIQUE
  fs_type       = "ext4"
  mount_point   = "/srv/docker" ##  MUST BE UNIQUE
}
