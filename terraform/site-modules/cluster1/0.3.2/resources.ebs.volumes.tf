...
data "template_file" "all_ebs_volume_detail_id_lists" {
  ...
  template      = "${
    join(
      ";",
      list(
        "${element(module.foo.volume_details_ids_list, count.index)}",
        "${element(module.boo.volume_details_ids_list, count.index)}",
        "${element(module.moo.volume_details_ids_list, count.index)}",
        "${element(module.zoo.volume_details_ids_list, count.index)}",
        "${element(module.roo.volume_details_ids_list, count.index)}"
      )
    )
  }"
}
...
module "foo" {
  ...
  vol_name      = "foo"      ##  IGNORED
  vol_label     = "foo"      ##  MUST BE UNIQUE
  fs_type       = "ext4"
  mount_point   = "/mnt/foo" ##  MUST BE UNIQUE
}

##  GP2, 50GB, ENCRYPTED WITH PROVIDED (CUSTOMER-MANAGED) KMS KEY
module "boo" {
  source        = "git::ssh://git@cgit01.bitpusher.com/bp-tools/bitpusher-terraform-modules//ec2/ebs.vol.attach/0.1.3"
  count         = "${var.instance_count}"
  instance_ids  = "${module.cluster.instance_ids}"
...
##  IO1, 200GB, 100 IOPS, ENCRYPTED WITH AWS-MANAGED KEY
module "roo" {
  source        = "git::ssh://git@cgit01.bitpusher.com/bp-tools/bitpusher-terraform-modules//ec2/ebs.vol.attach/0.1.3"
  count         = "${var.instance_count}"
  instance_ids  = "${module.cluster.instance_ids}"
