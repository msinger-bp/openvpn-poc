...
directory "/srv/acadience/frontend/pm2" do
  owner  node[cookbook_name]['container']['user']
  group  node[cookbook_name]['container']['group']
  mode   "0755"
  action :create
...
env << "SOCKETIO_REDIS=//#{node['socketio']['host']}:#{node['socketio']['port']}" if node['env_flags']['socketio_redis']==true
...
#    log_opts       [ 'max-size=10M', 'max-file=5' ]
    volumes        [ '/srv/acadience/frontend/config:/config', '/srv/acadience/frontend/data:/data', '/srv/acadience/frontend/tmp:/tmp', "/srv/acadience/frontend/pm2/frontend-#{i}:/app/.pm2" ]
    ro_rootfs      true
